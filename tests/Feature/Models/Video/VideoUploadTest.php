...
class VideoUploadTest extends BaseVideoTestCase
{
    ...
    public function testeCreateIfRollbackFiles()
    {
        \Storage::fake();
    ...
    public function testUpdateWithFiles()
    {
        ...
        $thumbFile = UploadedFile::fake()->image('thump.jpg');
    ...
    public function testUpdateIfRollbackFiles()
    {
        ...
        try {
            $video->update(
                $this->data + [
                    'thumb_file' => UploadedFile::fake()->create('thump.jpg'),
                    'video_file' => UploadedFile::fake()->image('video.mp4')
                ]
            );
        } catch (TestException $e) {
    ...
    public function testFileUrlsWithGcsDriver()
    {
        ...
        $video = factory(Video::class)->create($fileFields);
        $baseUrl = config('filesystems.disks.storage_api_url');
        \Config::set('filesystems.default', $baseUrl);
        foreach ($fileFields as $field => $value) {
            $fileUrl = $video->{"{$field}_url"};
            $this->assertEquals("{baseUrl}/$video->id/$value", $fileUrl);
        }
